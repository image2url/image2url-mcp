...
const DEFAULT_BASE = sanitizeBaseUrl(
  process.env.IMAGE2URL_BASE_URL ?? "https://www.image2url.com",
);
const DEFAULT_UPLOAD_PATH = normalizePath(
  process.env.IMAGE2URL_UPLOAD_PATH ?? "/api/upload",
);
const UPLOAD_URL =
  process.env.IMAGE2URL_UPLOAD_URL ?? `${DEFAULT_BASE}${DEFAULT_UPLOAD_PATH}`;
const API_TOKEN = process.env.IMAGE2URL_API_TOKEN;
const MAX_BYTES =
  Number.parseInt(process.env.IMAGE2URL_MAX_BYTES ?? "", 10) ||
  2 * 1024 * 1024; // align with hosted limit
const TIMEOUT_MS =
  Number.parseInt(process.env.IMAGE2URL_TIMEOUT_MS ?? "", 10) || 20_000;
...
const server = new McpServer(
  ...
  {
    instructions:
      "One-command setup: add {\"command\": \"npx\", \"args\": [\"-y\", \"mcp-server-image2url\"]} to your MCP client. No API key needed; default endpoint uses IP-based free quota at image2url.com. Override with IMAGE2URL_BASE_URL or IMAGE2URL_UPLOAD_URL when self-hosting.",
    capabilities: {
      logging: {},
    },
  },
...
server.registerTool(
  "upload_image",
  {
    ...
    description:
      "Upload a local file or remote image URL to image2url.com and get ready-to-paste snippets.",
...
async function readLocalFile(filePath: string): Promise<UploadSource> {
  ...
  if (stat.size > MAX_BYTES) {
    throw new Error(
      `File is too large. Limit: ${formatBytes(
        MAX_BYTES,
      )}, got ${formatBytes(stat.size)}`,
    );
  }
...
async function fetchRemoteImage(url: string): Promise<UploadSource> {
  ...
  if (contentLength && Number.parseInt(contentLength, 10) > MAX_BYTES) {
    throw new Error(
      `Remote image exceeds limit. Limit: ${formatBytes(
        MAX_BYTES,
      )}, reported ${formatBytes(Number.parseInt(contentLength, 10))}`,
    );
  }
  ...
  if (!contentType.startsWith("image/")) {
    throw new Error(
      `Only image content types are supported (detected ${contentType}).`,
    );
  }
...
async function uploadToImage2Url(source: UploadSource): Promise<UploadResponse> {
  const controller = AbortSignal.timeout(TIMEOUT_MS);
  const form = new FormData();
  const arrayBuffer = source.buffer.buffer.slice(
  ...
  if (responseText) {
    try {
      ...
      throw new Error(
        `Unexpected response from image2url: ${response.status} ${responseText}`,
      );
    }
  }
  ...
  if (!response.ok) {
    ...
    throw new Error(
      serverError || `Upload failed with status ${response.status}.`,
    );
  }
  ...
  return {
    url,
    uploadedAt:
      typeof data.uploadedAt === "string" ? data.uploadedAt : undefined,
  };
}

function buildSnippets(url: string, alt: string) {
  return {
    markdown: `![${alt}](${url})`,
...
function formatResponse(
  content: z.infer<typeof uploadOutputSchema>,
  format: z.infer<typeof uploadInputSchema>["format"],
) {
  const lines = [
    ...
    ...(format === "all" || format === "bbcode"
      ...
      : []),
    `Source: ${content.source} (${content.filename}, ${formatBytes(
      content.size,
    )}, ${content.mimeType})`,
...
async function start() {
  ...
  console.error(
    `mcp-server-image2url ${VERSION} ready on stdio -> ${UPLOAD_URL} (limit ${formatBytes(
      MAX_BYTES,
    )})`,
  );
}
